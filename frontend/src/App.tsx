      ...
      <aside className="fixed inset-y-0 left-0 hidden w-80 border-r border-line bg-[#0b1020]/95 px-7 py-8 backdrop-blur-xl lg:block">
        ...
        <nav className="mt-10 space-y-3">
          {nav.map(([Icon, label, target]) => (
            <button key={target} onClick={() => setPage(target)} className={`flex w-full items-center gap-4 rounded-2xl px-4 py-4 text-left text-lg font-bold transition ${page === target ? 'bg-accent/15 text-accent shadow-xl shadow-accent/5' : 'text-slate-300 hover:bg-white/5'}`}>
              <Icon size={22} />
              <span>{label}</span>
            </button>
          ))}
        ...
        <div className="fixed inset-0 z-30 bg-black/70 lg:hidden">
          <div className="h-full w-80 bg-[#0b1020] p-6">
            ...
            <nav className="mt-8 space-y-3">
              {nav.map(([Icon, label, target]) => (
                <button key={target} onClick={() => { setPage(target); setMobileOpen(false); }} className="flex w-full items-center gap-4 rounded-2xl px-4 py-4 text-left text-lg font-bold text-slate-200">
                  <Icon size={22} /> {label}
                </button>
              ))}
            </nav>
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-ink px-5 text-slate-100">
      <section className="w-full max-w-xl rounded-2xl border border-line bg-panel/90 p-8 shadow-xl shadow-cyan-500/10 backdrop-blur">
        <Brand />
        <h1 className="mt-8 text-4xl font-black text-white">{authMode === 'login' ? 'Hesabina giris yap' : 'Demo portfoyunu olustur'}</h1>
        <p className="mt-3 text-lg text-slate-400">{authMode === 'login' ? 'Token ile korunan TRadeTurk paneline devam et.' : 'Kayitta 50,000 USDT demo cüzdan ve sanal kart otomatik acilir.'}</p>
        ...
        <div className="mt-7 space-y-4">
          ...
          <button disabled={mutation.isPending} onClick={() => mutation.mutate()} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-accent px-5 py-4 text-lg font-black text-white shadow-xl shadow-accent/20 disabled:opacity-60">
    ...
    <>
        ...
        {symbols.map((symbol) => {
          ...
          return <article key={symbol} className="rounded-2xl border border-line bg-panel p-6 shadow-xl">
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-center gap-4"><div className="grid h-14 w-14 place-items-center rounded-2xl bg-white/5 text-xl font-black">{coinShort(symbol)[0]}</div><div><div className="text-2xl font-black">{coinShort(symbol)}</div><div className="text-base text-slate-400">{coinName(symbol)}</div></div></div>
              <div className={`text-right text-2xl font-black ${direction}`}>{formatCurrency(price.price)}</div>
            </div>
      ...
      <section className="grid gap-6 xl:grid-cols-[440px_1fr]">
        <div className="rounded-2xl border border-line bg-panel p-6 shadow-xl">
          <div className="mb-5 flex items-center justify-between"><h2 className="text-2xl font-black">Hizli Islem</h2>{isTrading && <Loader2 className="animate-spin text-accent" />}</div>
          <div className="mb-5 grid grid-cols-2 rounded-2xl bg-black/25 p-1.5">{(['buy', 'sell'] as TradeMode[]).map((item) => <button key={item} onClick={() => setMode(item)} className={`rounded-xl py-3 text-lg font-black ${mode === item ? 'bg-white/10 text-white' : 'text-slate-500'}`}>{item === 'buy' ? 'Alis' : 'Satis'}</button>)}</div>
          <label className="mb-2 block text-base font-semibold text-slate-300">Varlik</label>
          <select value={selectedSymbol} onChange={(event) => setSelectedSymbol(event.target.value)} className="mb-5 w-full rounded-2xl border border-line bg-panelSoft px-4 py-4 text-lg outline-none">{symbols.map((symbol) => <option key={symbol} value={symbol}>{coinName(symbol)} ({coinShort(symbol)})</option>)}</select>
          <label className="mb-2 block text-base font-semibold text-slate-300">Miktar</label>
          <div className="mb-5 flex items-center rounded-2xl border border-line bg-panelSoft px-4"><input value={amount} onChange={(event) => setAmount(event.target.value)} type="number" min="0" step="0.00000001" className="w-full bg-transparent py-4 text-lg outline-none" placeholder="0.00" /><span className="text-base font-black text-slate-400">{coinShort(selectedSymbol)}</span></div>
          ...
          <button disabled={isTrading} onClick={submitTrade} className={`w-full rounded-2xl py-4 text-lg font-black text-white shadow-xl disabled:opacity-60 ${mode === 'buy' ? 'bg-accent shadow-accent/20' : 'bg-danger shadow-danger/20'}`}>{mode === 'buy' ? 'Satin Alimi Onayla' : 'Satisi Onayla'}</button>
        </div>
...
function TransactionsPage({ isLoading, transactions }: any) {
  ...
  return <section className="rounded-2xl border border-line bg-panel p-6 shadow-xl">
    <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><h1 className="text-3xl font-black">Islem Gecmisi</h1><div className="grid grid-cols-3 rounded-2xl bg-black/25 p-1.5">{(['ALL', 'BUY', 'SELL'] as const).map((item) => <button key={item} onClick={() => setFilter(item)} className={`rounded-xl px-4 py-2 text-base font-black ${filter === item ? 'bg-white/10 text-white' : 'text-slate-500'}`}>{item === 'ALL' ? 'Tumu' : item === 'BUY' ? 'Alis' : 'Satis'}</button>)}</div></div>
...
function AssetList({ assets, prices }: any) {
  return <div className="rounded-2xl border border-line bg-panel p-6 shadow-xl"><h2 className="mb-5 text-2xl font-black">Varliklarim</h2><div className="space-y-4">{assets.length === 0 && <State text="Henuz varlik yok." />}{assets.map((asset: any) => <div key={asset.id} className="grid gap-4 rounded-2xl border border-line bg-white/[0.04] p-5 text-base sm:grid-cols-4"><div><div className="text-xl font-black">{coinShort(asset.symbol)}</div><div className="text-slate-500">{asset.symbol}</div></div><Info label="Miktar" value={asset.amount} /><Info label="Ortalama" value={formatCurrency(asset.averageCost)} /><Info label="Deger" value={formatCurrency(asset.amount * (prices[asset.symbol]?.price ?? asset.averageCost))} /></div>)}</div></div>;
}
...
function UserPanel({ user, onLogout }: { user?: UserDto; onLogout: () => void }) { return <div className="absolute bottom-7 left-7 right-7 rounded-2xl border border-line bg-white/[0.04] p-5"><div className="mb-4 flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-slate-700 text-lg font-black">{user?.fullName?.slice(0, 2).toUpperCase() ?? 'TT'}</div><div><div className="text-lg font-black">{user?.fullName ?? 'TRadeTurk'}</div><div className="text-sm text-slate-500">{user?.email}</div></div></div><button onClick={onLogout} className="flex w-full items-center justify-center gap-2 rounded-xl border border-line py-3 font-bold text-slate-300"><LogOut size={18} /> Cikis</button></div>; }
function Metric({ title, value, sub }: { title: string; value: string; sub: string }) { return <article className="rounded-2xl border border-line bg-panel p-6 shadow-xl"><div className="text-lg font-semibold text-slate-400">{title}</div><div className="mt-3 text-3xl font-black text-white">{value}</div><div className="mt-3 text-base text-cyan-300">{sub}</div></article>; }
