...
export function App() {
  ...
  return (
    <div className="min-h-screen bg-ink text-sm text-slate-100">
      ...
      <aside className="fixed inset-y-0 left-0 hidden w-80 border-r border-line bg-[#0b1020]/95 px-7 py-8 backdrop-blur-xl lg:block">
        ...
        <nav className="mt-10 space-y-3">
          {nav.map(([Icon, label, target]) => (
            <button key={target} onClick={() => setPage(target)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-bold transition ${page === target ? 'bg-accent/15 text-accent shadow-xl shadow-accent/5' : 'text-slate-300 hover:bg-white/5'}`}>
              <Icon size={22} />
              <span>{label}</span>
            </button>
          ))}
        ...
        <div className="fixed inset-0 z-30 bg-black/70 lg:hidden">
          <div className="h-full w-80 bg-[#0b1020] p-6">
            ...
            <nav className="mt-8 space-y-3">
              {nav.map(([Icon, label, target]) => (
                <button key={target} onClick={() => { setPage(target); setMobileOpen(false); }} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-bold text-slate-200">
                  <Icon size={22} /> {label}
                </button>
              ))}
            </nav>
      ...
      <main className="lg:pl-80">
        <header className="sticky top-0 z-20 border-b border-line bg-ink/85 px-5 py-4 backdrop-blur-xl lg:px-8">
          <div className="flex items-center justify-between gap-4">
            ...
            <button className="grid h-10 w-10 place-items-center rounded-xl border border-line bg-white/[0.04]"><Bell size={18} /></button>
          </div>
        </header>
...
function SettingsPage({ user, onSaved, setNotice }: { user?: UserDto; onSaved: (user: UserDto) => void; setNotice: (notice: { type: 'success' | 'error'; message: string }) => void }) {
  ...
  return <section className="grid gap-6 xl:grid-cols-2"><div className="rounded-2xl border border-line bg-panel p-5 shadow-xl"><h1 className="mb-5 text-lg font-black">Ayarlar</h1><div className="space-y-4"><Input label="Ad soyad" value={profile.fullName} onChange={(fullName) => setProfile({ ...profile, fullName })} /><Input label="Email" value={profile.email} onChange={(email) => setProfile({ ...profile, email })} /><Input label="Kullanici adi" value={profile.userName} onChange={(userName) => setProfile({ ...profile, userName })} /><Input label="Para birimi" value={profile.preferredCurrency} onChange={(preferredCurrency) => setProfile({ ...profile, preferredCurrency })} /><Input label="Tema" value={profile.themePreference} onChange={(themePreference) => setProfile({ ...profile, themePreference })} /><button onClick={() => saveProfile.mutate()} className="rounded-xl bg-accent px-5 py-3 text-sm font-black text-white">Kaydet</button></div></div><div className="rounded-2xl border border-line bg-panel p-5 shadow-xl"><h2 className="mb-5 text-base font-black">Sifre Degistir</h2><div className="space-y-4"><Input label="Mevcut sifre" type="password" value={passwords.currentPassword} onChange={(currentPassword) => setPasswords({ ...passwords, currentPassword })} /><Input label="Yeni sifre" type="password" value={passwords.newPassword} onChange={(newPassword) => setPasswords({ ...passwords, newPassword })} /><button onClick={() => savePassword.mutate()} className="rounded-xl bg-cyan-500 px-5 py-3 text-sm font-black text-white">Sifreyi Guncelle</button></div></div></section>;
}

function AssetList({ assets, prices }: any) {
  return <div className="rounded-2xl border border-line bg-panel p-5 shadow-xl"><h2 className="mb-5 text-base font-black">Varliklarim</h2><div className="space-y-4">{assets.length === 0 && <State text="Henuz varlik yok." />}{assets.map((asset: any) => <div key={asset.id} className="grid gap-4 rounded-xl border border-line bg-white/[0.04] p-4 text-sm sm:grid-cols-4"><div><div className="text-sm font-black">{coinShort(asset.symbol)}</div><div className="text-xs text-slate-500">{asset.symbol}</div></div><Info label="Miktar" value={asset.amount} /><Info label="Ortalama" value={formatCurrency(asset.averageCost)} /><Info label="Deger" value={formatCurrency(asset.amount * (prices[asset.symbol]?.price ?? asset.averageCost))} /></div>)}</div></div>;
}
